...
local events=
{
    ...
    EventHandler("transform_werelizard", function(inst, data)
        if inst.components.lizardness then
           -- TheCamera:SetDistance(14)
            inst.sg:GoToState("werelizard")
...
--]]
    ...
    EventHandler("death", function(inst)
        ...
        inst.SoundEmitter:PlaySound("wik/characters/wik/death", "death")
    ...
    EventHandler("ontalk", function(inst, data)
     ...
     EventHandler("wikintro",
        function(inst)
            inst.sg:GoToState("wikintro")
        end),       
    EventHandler("wakeup",
        function(inst)
...
local states= 
{
    State{
        ...
        onenter = function(inst)
            ...
            local thoughts = SpawnPrefab("wikthought")
            thoughts:Hide()
            thoughts.Transform:SetPosition(GetPlayer():GetPosition().x,GetPlayer():GetPosition().y,GetPlayer():GetPosition().z)     
            TheCamera:SetDistance(10)
            inst.components.talker.fontsize = 50
            inst.components.talker.offset = Vector3(0,-450,0)

            inst:DoTaskInTime(2.5, function()
                inst.components.talker:Say("What happen??")
                inst.SoundEmitter:PlaySound("wik/characters/wik/talk_LP")
                inst.AnimState:PlayAnimation("idle_inaction_sanity")
                TheCamera:Shake("SIDE", 0.5, 0.05, 0.1)
                inst.components.sanity:SetPercent(.65)
            end)
            inst:DoTaskInTime(4.5, function()
                inst:FacePoint(GetPlayer():GetPosition().x-1,GetPlayer():GetPosition().y,GetPlayer():GetPosition().z-4)
                TheCamera:SetDistance(9)
                TheCamera:SetOffset(Vector3(0,3,0))
            end)
            inst:DoTaskInTime(6.1, function()
                inst.components.talker:Say("Skinny curly tail put me in cage!")
                inst.SoundEmitter:PlaySound("wik/characters/wik/talk_LP")
            end)
            inst:DoTaskInTime(8, function()
                TheCamera:SetOffset(Vector3(0,2,0))
                TheCamera:SetDistance(6)
                TheCamera:Snap()
                inst:FacePoint(GetPlayer():GetPosition().x,GetPlayer():GetPosition().y-1,GetPlayer():GetPosition().z)
                TheCamera:Shake("SIDE", 0.5, 0.05, 0.1)
                thoughts.AnimState:PushAnimation("meat", true)
                inst.components.talker:Say("Argh!!!! And me eat meat!")
                inst.SoundEmitter:PlaySound("wik/characters/wik/hurt")
            end)
            inst:DoTaskInTime(10.4, function()
                thoughts:Hide()
                TheCamera:SetDistance(8)
                -- Add merm sad face
                inst.components.talker:Say("Cousins said meat yucky...")
                inst.SoundEmitter:PlaySound("wik/characters/wik/talk_LP")
            end)
            inst:DoTaskInTime(12.2, function()
                inst.components.talker:Say("But meat GOOD, make me strong!")
                inst.SoundEmitter:PlaySound("wik/characters/wik/talk_LP")
            end)
            inst:DoTaskInTime(12.7, function()
                inst.AnimState:PlayAnimation("powerup")
            end)
            inst:DoTaskInTime(14, function()
                TheCamera:SetDistance(10)
                TheCamera:SetOffset(Vector3(4,0,0))
            end)
            inst:DoTaskInTime(14.5, function()
                thoughts.AnimState:PushAnimation("pig", true)
                thoughts:Show()
                inst.components.talker.offset = Vector3(-250,-100,0)
                inst.components.talker:Say("He put me in cage...")
                inst.SoundEmitter:PlaySound("wik/characters/wik/talk_LP")
            end)
            inst:DoTaskInTime(16.1, function() 
                inst.components.talker.offset = Vector3(0,-450,0)
                inst.components.talker:Say("Me make all curly tails bones now!!")
                TheCamera:SetOffset(Vector3(0,2,0))
            end)
            inst:DoTaskInTime(19.6, function()
                thoughts.AnimState:PushAnimation("merm", true)
                inst.components.talker:Say("Maybe cousins help ... me go find them")
                inst.SoundEmitter:PlaySound("wik/characters/wik/talk_LP")
            end)
            inst:DoTaskInTime(21.5, function()
                thoughts:Remove()
            end)
            inst:DoTaskInTime(35, function()
                inst.components.talker:Say("COUSINS?")
                inst.SoundEmitter:PlaySound("wik/characters/wik/talk_LP")
            end)
            inst:DoTaskInTime(45, function()
                inst.components.talker:Say("Me not go in cage again")
                inst.SoundEmitter:PlaySound("wik/characters/wik/talk_LP")
            end)
        end,
        
        onexit = function(inst)
            inst:DoTaskInTime(16.9, function()
                TheCamera:SetDefault()
                inst.components.playercontroller:Enable(true)
                inst.components.health:SetInvincible(false)
                inst.HUD:Show()
                inst:RemoveTag("PlayingIntro")
    ...
    State{ name = "chop_start",
        ...
        onenter = function(inst)
            inst.components.locomotor:Stop()
            inst.AnimState:PlayAnimation("chop_pre") -- inst.prefab == "wik" and "wik_chop_pre" or  
        end,
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.AnimState:PlayAnimation("chop_loop") -- inst.prefab == "wik" and "wik_chop_loop" or             
        end,
...
--]]                
   ...
   State{
    ...
    State{
        ...
        onexit = function(inst)
			if not inst.components.lizardness:IsLizard() then
...
return StateGraph("wik", states, events, "idle", actionhandlers)
