...
-- and v.components.workable.workleft > 0
...
local actionhandlers = 
{
    ...
    ActionHandler(ACTIONS.HAMMER, "pound"),
...
local events=
{
    EventHandler("locomote", function(inst)
        ...
        local should_run = (inst.shallwalk == 0)
...
local function ShakeIfClose(inst)
        ...
        inst.components.playercontroller:ShakeCamera(inst, "FULL", 0.7, 0.02, 2.6, 10)

end

local function ShakeIfClose_Footstep(inst)
    if inst.shallwalk == 1 and not inst.components.combat.target then
        inst.components.playercontroller:ShakeCamera(inst, "FULL", 0.35, 0.02, 0.33, 5)
	else
        inst.components.playercontroller:ShakeCamera(inst, "FULL", 0.35, 0.02, 0.6, 5)
	end
end

local function DoFootstep(inst)
    if inst.shallwalk == 1 and not inst.components.combat.target then --inst:IsStandState("quad") then
...
-------------------EATING-------------------------
    ...
    State{
        ...
        timeline=
        {
            TimeEvent(3*FRAMES, function(inst) if inst.sg.statemem.firstsound then inst.SoundEmitter:PlaySound("dontstarve_DLC001/creatures/bearger/chew") else inst.sg.statemem.firstsound = true end end),
...
---------------------------WALKING---------------
    ...
    State{
            ...
            onenter = function(inst) 
                local anim = (inst.shallwalk == 2 or inst.components.combat.target) and "charge_pre" or "walk_pre"
                if GoToStandState(inst, "bi") then
    ...
    State{            
            ...
            onenter = function(inst)
                local anim = (inst.shallwalk == 2 or inst.components.combat.target) and "charge_loop" or "walk_loop"
                if GoToStandState(inst, "bi") then
                    inst.AnimState:PlayAnimation(anim)
				end
				if anim == "charge_loop" then
    ...
    State{            
            ...
            onenter = function(inst) 
                inst.components.locomotor:StopMoving()
                local anim = (inst.shallwalk == 2 or inst.components.combat.target) and "charge_pst" or "walk_pst"
                DoFootstep(inst)
                if GoToStandState(inst, "bi") then
                    inst.AnimState:PlayAnimation(anim)
				end
...
return StateGraph("beargerPlayer", states, events, "idle", actionhandlers)
